...
import static com.extra.utils.FileUtils.getDataColumn;
import static com.extra.utils.FileUtils.isDownloadsDocument;
import static com.extra.utils.FileUtils.isExternalStorageDocument;
import static com.extra.utils.FileUtils.isGooglePhotosUri;
import static com.extra.utils.FileUtils.isMediaDocument;
...
public class PhotoUtils {
    ...
    public static void openCameraImage(final Activity activity, String fileUrl) {
        imageUriFromCamera = createImagePathUri(activity,fileUrl);
    ...
    public static void cropImage(Activity activity, Uri srcUri, String str) {
        cropImageUri = createImagePathUri(activity,str);
     ...
     * @return
     */
    public static Uri createImagePathUri(final Context context, String str) {
        final Uri[] imageFilePath = {null};
        ...
        if (ContextCompat.checkSelfPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            ...
            if (status.equals(Environment.MEDIA_MOUNTED)) {// 判断是否有SD卡,优先使用SD卡存储,当没有SD卡时使用手机存储
                imageFilePath[0] = Uri.fromFile(new File(str, imageName + ".jpeg"));
            } else {
                imageFilePath[0] = Uri.fromFile(new File(str, imageName + ".jpeg"));
            }
        }
